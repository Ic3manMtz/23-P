<?php
    class Paginar {
        ...
        private $conexion;
        private $limite = 7; // Número de resultados por página
        ...
        public function __construct($conex) {     
            $this->conexion = $conex;
            $this->consulta = "SELECT e.id, e.nombre, e.ap_paterno, e.ap_materno, e.fecha_nacimiento, e.direccion, e.fecha_alta, e.fecha_baja, e.salario, p.nombre AS puesto, d.nombre AS departamento FROM empleado AS e INNER JOIN puesto AS p ON e.puesto=p.id INNER JOIN departamento as d ON p.departamento=d.id ORDER BY fecha_alta ASC ";
