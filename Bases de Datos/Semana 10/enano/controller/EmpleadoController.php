...
class EmpleadoController implements EmpleadoDAO{
    ...
    public function updateEmpleado(Empleado $empleado){
        ...
        $stmt->bind_param("sssssssssi", $nombre, $apPaterno, $apMaterno, $fecNacimiento, $direccion, $fecAlta, $fecBaja, $salario, $puesto, $id);
        
        if ($stmt->execute()) {
            // Si la consulta se ejecutó correctamente, imprimir un mensaje en pantalla
            echo "El empleado fue actualizado correctamente en la base de datos.";
        } else {
            // Si la consulta no se ejecutó correctamente, imprimir un mensaje de error
            echo "Hubo un error al actualizar el empleado en la base de datos: " . $stmt->error;
        }
    }
}
    ?>